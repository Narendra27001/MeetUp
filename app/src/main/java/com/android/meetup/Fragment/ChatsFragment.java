        ...
        myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot snap:snapshot.getChildren()) {
                    ...
                    if(chat.getInterest().equals(cUser.getInterest())) mChatList.add(chat);
                }
                mUsers=new ArrayList<>();
                myRef=FirebaseDatabase.getInstance().getReference(Parameters.MyUsers.toString());
                myRef.addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        mUsers.clear();
                        for(DataSnapshot snap: snapshot.getChildren()){
                            ...
                            for(ChatList chat: mChatList){
                                if(chat.getId().equals(users.getId()) && chat.getInterest().equals(users.getInterest())){
                                    mUsers.add(users);
                                }
                            }
                        }
